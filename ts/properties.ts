...
window.wallpaperPropertyListener = {
    applyUserProperties: _props => (async (newProps) => {
        ...
        /* Clock Options */
        {
            const clockBackground = clockElement.querySelector<HTMLDivElement>(".background")!;
            const hrFormatElement = document.querySelector<HTMLDivElement>("#hrFormat")!;
            const secondsSeparatorElement = document.querySelector<HTMLSpanElement>("#secondsSeparator")!;
            const secondsElement = document.querySelector<HTMLDivElement>("#seconds")!;
        ...
        /* Calendar Options */
        {
            const calendarBackground = calendarElement.querySelector<HTMLDivElement>(".background")!;
            const dayNameElement = document.querySelector<HTMLDivElement>("#dayName")!;
            const dayElement = document.querySelector<HTMLDivElement>("#day")!;
            const monthElement = document.querySelector<HTMLDivElement>("#month")!;
            const yearElement = document.querySelector<HTMLDivElement>("#year")!;
        ...
        /* Weather Options */
        {
            const weatherBackground = weatherElement.querySelector<HTMLDivElement>(".background")!;
        ...
        /* Colors */
        {
            if (newProps.dynamictemperaturecolor != undefined) {
                if (!newProps.dynamictemperaturecolor) {
                    document.querySelector<HTMLDivElement>("#temperature")!.style.color = "";
                }
                else {
                    shouldUpdateWeather = true;
                }
            ...
            if (newProps.clockseparatorcolor != undefined) {
                document.querySelectorAll<HTMLDivElement>("#clock span").forEach(element => element.style.setProperty("--color", parseColor(newProps.clockseparatorcolor ?? props.clockseparatorcolor)));
            }

            if (newProps.hourscolor != undefined) {
                document.querySelector<HTMLDivElement>("#hours")!.style.setProperty("--color", parseColor(newProps.hourscolor ?? props.hourscolor));
            }

            if (newProps.minutescolor != undefined) {
                document.querySelector<HTMLDivElement>("#minutes")!.style.setProperty("--color", parseColor(newProps.minutescolor ?? props.minutescolor));
            }

            if (newProps.secondscolor != undefined) {
                document.querySelector<HTMLDivElement>("#seconds")!.style.setProperty("--color", parseColor(newProps.secondscolor ?? props.secondscolor));
            }

            if (newProps.ampmcolor != undefined) {
                document.querySelector<HTMLDivElement>("#hrFormat")!.style.setProperty("--color", parseColor(newProps.ampmcolor ?? props.ampmcolor));
            }

            if (newProps.daynamecolor != undefined) {
                document.querySelector<HTMLDivElement>("#dayName")!.style.setProperty("--color", parseColor(newProps.daynamecolor ?? props.daynamecolor));
            }

            if (newProps.daycolor != undefined) {
                document.querySelector<HTMLDivElement>("#day")!.style.setProperty("--color", parseColor(newProps.daycolor ?? props.daycolor));
            }

            if (newProps.monthcolor != undefined) {
                document.querySelector<HTMLDivElement>("#month")!.style.setProperty("--color", parseColor(newProps.monthcolor ?? props.monthcolor));
            }

            if (newProps.yearcolor != undefined) {
                document.querySelector<HTMLDivElement>("#year")!.style.setProperty("--color", parseColor(newProps.yearcolor ?? props.yearcolor));
            }
            ...
            if (newProps.hoursfill != undefined) {
                document.querySelector<HTMLDivElement>("#hours")!.style.setProperty("--fillColor", parseColor(newProps.hoursfill ?? props.hoursfill));
            }

            if (newProps.minutesfill != undefined) {
                document.querySelector<HTMLDivElement>("#minutes")!.style.setProperty("--fillColor", parseColor(newProps.minutesfill ?? props.minutesfill));
            }

            if (newProps.secondsfill != undefined) {
                document.querySelector<HTMLDivElement>("#seconds")!.style.setProperty("--fillColor", parseColor(newProps.secondsfill ?? props.secondsfill));
            }

            if (newProps.ampmfill != undefined) {
                document.querySelector<HTMLDivElement>("#hrFormat")!.style.setProperty("--fillColor", parseColor(newProps.ampmfill ?? props.ampmfill));
            }

            if (newProps.daynamefill != undefined) {
                document.querySelector<HTMLDivElement>("#dayName")!.style.setProperty("--fillColor", parseColor(newProps.daynamefill ?? props.daynamefill));
            }

            if (newProps.dayfill != undefined) {
                document.querySelector<HTMLDivElement>("#day")!.style.setProperty("--fillColor", parseColor(newProps.dayfill ?? props.dayfill));
            }

            if (newProps.monthfill != undefined) {
                document.querySelector<HTMLDivElement>("#month")!.style.setProperty("--fillColor", parseColor(newProps.monthfill ?? props.monthfill));
            }

            if (newProps.yearfill != undefined) {
                document.querySelector<HTMLDivElement>("#year")!.style.setProperty("--fillColor", parseColor(newProps.yearfill ?? props.yearfill));
            }

            if (newProps.temperaturefill != undefined) {
                document.querySelector<HTMLDivElement>("#temperature")!.style.setProperty("--fillColor", parseColor(newProps.temperaturefill ?? props.temperaturefill));
            }

            if (newProps.weatherfill != undefined) {
                document.querySelector<HTMLDivElement>("#weatherName")!.style.setProperty("--fillColor", parseColor(newProps.weatherfill ?? props.weatherfill));
                document.querySelector<HTMLDivElement>("#location")!.style.setProperty("--fillColor", parseColor(newProps.weatherfill ?? props.weatherfill));
            }

            if (newProps.differentcolors === false) {
                document.querySelectorAll<HTMLDivElement>(elementIDs).forEach(element => element.style.setProperty("--color", ""));
            }

            if (newProps.differentfillcolors === false) {
                document.querySelectorAll<HTMLDivElement>(elementIDs).forEach(element => element.style.setProperty("--fillColor", ""));
            }
        }

        if (shouldUpdate) update();
        if (shouldUpdateWeather) updateWeather();
    })(parseProps(_props)),

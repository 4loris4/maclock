abstract class WallpaperElement {
    readonly element: HTMLDivElement;
    readonly adjustPercentage: (percentage: number) => string;

    constructor(id: string, adjustPercentageFunction = adjustPercentage()) {
        this.element = document.querySelector(id)!;
        this.adjustPercentage = adjustPercentageFunction;
    }

    abstract update(date: Date, futureDate: Date): void;

    async updateAnimation(value: number, maxValue: number, linearValue: number, maxLinearValue: number, futureValue: number) {
...
abstract class WeatherElement {
    ...
    abstract update(weather: WeatherData): void;
}
...
function parseProps(props: { [key: string]: { value: number | boolean | string; }; }): Partial<Properties> {
    return Object.fromEntries(Object.entries(props).map(([key, { value }]) => [key, value]));
}

function adjustPercentage(min = 0, max = 1): (percentage: number) => string {
...
function getPassedDaysInYear(year: number, month: number): number {
    let days = 0;
...
function setAlignment(element: HTMLElement, alignment: AlignmentType) {
    switch (alignment) {
